...
__global__ void merge_sort(float *d_unsorted_arr, float *d_sorted_arr, unsigned long long length, unsigned long long chunk)
{
	...
	while (left < middle || right < end)
	{
		...
		{
			result = d_unsorted_arr[left] <= d_unsorted_arr[right] ? d_unsorted_arr[left++] : d_unsorted_arr[right++];
		} else 
		{
...
void stopTimer()
{
	...
	cout << timed_operation << " took " << chrono::duration <double, milli>(diff).count() << " ms" << endl;
}

void cudaMergeSort(float *d_unsorted_array, float *d_sorted_array, unsigned long long length)
{
	...
	while (!isSorted)
	{
		...
		if (grids > 0)
		{
			merge_sort << <grids, threads_per_block >> >(d_unsorted_array, d_sorted_array, length, chunk);
		}
		else
		{
			merge_sort << <1, threads >> >(d_unsorted_array, d_sorted_array, length, chunk);
		}
		if (chunk >= length)
		{
			isSorted = true;
...
void cpuMergeSort(float *h_unsorted_array, float *h_sorted_array, unsigned long long start, unsigned long long chunk, unsigned long long length)
{
	unsigned long long middle = min(start + chunk / 2, length);
	unsigned long long end = min(start + chunk, length);
	unsigned long long left = start;
	...
	while (left < middle || right < end)
	{
		...
		if (left < middle && right < end)
		{
			result = h_unsorted_array[left] <= h_unsorted_array[right] ? h_unsorted_array[left++] : h_unsorted_array[right++];
		}
		else
		{
			result = left < middle ? h_unsorted_array[left++] : h_unsorted_array[right++];
		}
...
void cpuMergeSort(float *h_unsorted_array, float *h_sorted_array, unsigned long long length)
{
	unsigned long long chunk = 2;
	bool isSorted = false;
	while (!isSorted)
...
void checkSortedArray(float *sorted_array, unsigned long long length)
{
	...
	cout << "List size: " << length << ", Is correct: " << (isCorrect ? "Yes" : "No") << endl;
}
...
int main()
{
	...
	unsigned long long length = 10000000*5; // CPU takes 15 seconds at this length
